...
export const getUniqueStates = (country?: "US" | "CA"): string[] => {
  const filtered = country ? locationData.filter((loc) => loc.country === country) : locationData;
...
export const getFilteredData = (filters: Partial<FilterState>): LocationData[] => {
  return locationData.filter((location) => {
    ...
    if (filters.communities?.length) {
      const hasMatchingCommunity = filters.communities.some((c) => location.communities.includes(c));
  ...
  return {
    ...
    features: data.map((loc) => ({
      ...
      properties: {
        ...
        // Stringify arrays/objects so they can be properly parsed later
        communities: JSON.stringify(loc.communities),
        pointsOfInterest: JSON.stringify(loc.pointsOfInterest),
      },
      geometry: {
        type: "Point",
...
export const getStats = (data: LocationData[]) => {
  ...
  const totalPOIs = data.reduce(
    (sum, loc) => sum + loc.pointsOfInterest.reduce((poiSum, poi) => poiSum + poi.count, 0),
    0,
  );
